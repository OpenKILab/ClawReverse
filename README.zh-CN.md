...
- `delete`：删除某个 checkpoint 树根节点下面的所有后代资源，但保留你指定的根 checkpoint 本身。
...
如果你想先查看已经发布的 skill 页面，可以访问 [ClawHub 上的 ClawReverse](https://clawhub.ai/BinHuangPJLAB/clawreverse)。
...
### 5) 删除某个 checkpoint 根节点下面的全部后代资源

```bash
openclaw reverse delete --node <checkpoint-id>
```

这个命令会删除该树根下面的后代 checkpoint、branch 记录、子 session 资源以及子 agent 资源，但不会删除你传入的根 checkpoint。
